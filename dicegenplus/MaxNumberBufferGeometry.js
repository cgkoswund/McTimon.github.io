...
let MaxNumberBufferGeometry = {
    make: function(imageRes,yPoints){
        ...
        let innerMakePokedQuad = function(){
            ...
            cubeVerts.push(...MeshXTriOps.makePokedFace(
        ...
        for(let i = 0; i < tessRez[0]-1;i++){
            for(let j = 0; j < tessRez[1]-1;j++){//-tessRess/2 to center it. from uWidth*i - uWidth*iMax/2
                let yA = ((i+0)+(j*tessRez[0]))*4;
                let yB = ((i+1)+(j*tessRez[0]))*4;
                let yC = ((i+1)+((j+1)*tessRez[0]))*4;
                let yD = ((i+0)+((j+1)*tessRez[0]))*4;
                vertAOps = [unitSquareWidth*(i-tessRez[0]/2),(255-yPoints[yA])/2,unitSquareWidth*(j-tessRez[1]/2)]
                vertBOps = [unitSquareWidth*(i+1-tessRez[0]/2),(255-yPoints[yB])/2,unitSquareWidth*(j-tessRez[1]/2)]
                vertCOps = [unitSquareWidth*(i+1-tessRez[0]/2),(255-yPoints[yC])/2,unitSquareWidth*(j+1-tessRez[1]/2)]
                vertDOps = [unitSquareWidth*(i-tessRez[0]/2),(255-yPoints[yD])/2,unitSquareWidth*(j+1-tessRez[1]/2)]
                innerMakePokedQuad();
            }
        }
