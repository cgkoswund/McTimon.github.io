...
let oldLoadingIcon, newLoadingIcon;
...
function init() {
  ...
  let controlSphereOffsetZ =
    DefaultConstants.ringWidth - DefaultConstants.sphereSmallOffset;
  let controlSphereOffsetR =
    DefaultConstants.ringThickness - DefaultConstants.sphereSmallOffset;
      ...
      function makeSphere() {
        ...
        if (i % 4 == 0) {
          sphereMesh.position.set(
            (ringRadius + controlSphereOffsetR) * Math.cos(theta * i),
            (ringRadius + controlSphereOffsetR) * Math.sin(theta * i),
            controlSphereOffsetZ
          );
          curvePoints.push(
            new THREE.Vector3(
              (ringRadius + controlSphereOffsetR) * Math.cos(theta * i),
              (ringRadius + controlSphereOffsetR) * Math.sin(theta * i),
              controlSphereOffsetZ + curveSmallOffset
            )
          );
        }
      }
  ...
  scene.add(RingBufferGeo.platformBase());
  //   scene.add(RingBufferGeo.platformLoader(180));
...
function animate() {
  ...
  frameCounter++;
  updateLoadingIcon(frameCounter % 1200);
  ...
  if (selectedHandle && isTransforming) {
    ...
    //move all 4 spheres
    for (let i = 0; i < 4; i++) {
      handlesArray[batchSlot * 4 + i].position.z =
        selectedHandleLocZ * (1 - 2 * (i % 2));
    }
  }
  ...
  if (selectedHandle)
    selectedHandle.material = new THREE.MeshBasicMaterial({ color: 0xff66ae });
  ...
  for (let i = 0; i < intersects.length; i++) {
    ...
    if (isTransforming && selectedHandle) {
      selectedHandle.position.z = newCurveHandlePoint.z;
    }
  }
...
function updateLoadingIcon(timeInFrames) {
  oldLoadingIcon = newLoadingIcon;
  newLoadingIcon = RingBufferGeo.platformLoader(timeInFrames);
  scene.add(newLoadingIcon);
  if (oldLoadingIcon) {
    scene.remove(oldLoadingIcon);
    oldLoadingIcon.geometry.dispose();
    oldLoadingIcon.material.dispose();
    oldLoadingIcon = null;
  }
}
...
function toggleShade() {
  ...
  toggleOn = !toggleOn;
  if (toggleOn) {
    newRing.material = new THREE.MeshStandardMaterial({
      color: 0xffcc55,
      roughness: 0,
      metalness: 1,
    });
